...
const userAvatar = computed(() => user.value?.profileImageUrl || '');
...
<template>
  ...
  <LoadingScreen v-if="!isInitialized" >
    ...
    <div class="relative flex h-8 w-8 animate-spin rounded-full border-4 border-solid border-current border-r-transparent align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite]" role="status">
  ...
  <template v-else>
        ...
        <nav class="flex flex-1 flex-col">
          <ul role="list" class="flex flex-1 flex-col gap-y-7">
            ...
            <Menu as="div" class="mt-auto -mx-6" >
              ...
              <MenuButton class="flex items-center relative w-full gap-x-4 px-6 py-3 text-sm/6 font-semibold text-white hover:bg-neutral-800">
              ...
              >
                <MenuItems class="absolute left-3 z-10 mb-12 w-[90%] origin-bottom-right rounded-md bg-neutral-800 py-1 ring-1 shadow-lg ring-white/10 focus:outline-none sm:-translate-y-[calc(100%+3.5rem)]">
                  <MenuItem as="button" class="w-full" v-for="item in userNavigation" :key="item.name" v-slot="{ active, close }">
                    <RouterLink
                        :to="item.path"
    ...
    <div class="sticky top-0 z-40 flex items-center gap-x-6 px-4 py-4 shadow-xs sm:px-6 lg:hidden">
      ...
      <Menu as="div" class="relative">
        ...
        <transition
          ...
          leave-to-class="transform opacity-0 scale-95"
        >
          <MenuItems class="absolute right-0 z-10 mt-2.5 w-44 origin-top-right rounded-md bg-white dark:bg-neutral-800  py-2 ring-1 shadow-lg ring-gray-900/5 focus:outline-hidden">
            <MenuItem as="button" class="w-full" v-for="item in userNavigation" :key="item.name" v-slot="{ active, close }">
              <RouterLink
                :to="item.path"
                class="flex gap-2 items-center"
    ...
    <main class="sm:p-8 lg:pl-80 h-available">
      <div class="px-4 sm:px-6 lg:px-8 h-available grid">
        <RouterView />
      </div>
    </main>
  </template>
