...
onUnmounted(() => {
  ...
  disposeChatEmbed(channel.value.id);
...
<template>
  <div v-if="channel" class="min-h-[calc(100dvh+8.1rem)] max-h-[calc(100dvh+8.1rem)] sm:min-h-[calc(100dvh+12.1rem)] sm:max-h-[calc(100dvh+12.1rem)] w-available flex flex-col overflow-clip">
    ...
    <div class="flex flex-col sm:flex-row flex-1 h-[50dvh] w-available relative p-2 justify-between overflow-clip">
      ...
      <ChannelChat
        v-for="[key, chatEmbed] in chatEmbeds.entries()"
        :key="key"
        :chatEmbed="chatEmbed as ChatEmbed" />
    </div>
  </div>
</template>
